...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # compute custom loss (suppose one has several labels with different weights)
        loss_fct = nn.CrossEntropyLoss(weight=torch.tensor(class_weights).float().cuda())
...
if __name__ == "__main__":
	...
	class_weights = compute_class_weight(class_weight='balanced',classes=np.unique(y_train), y=np.stack(y_train, axis=0))
